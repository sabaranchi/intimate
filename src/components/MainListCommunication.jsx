  ...
  return <div className="main-list communication-list">
    ...
    <ul className="people-list">{sorted.map(person=>{
      ...
      return <li className="person-row communication-person-row" key={person.id} onClick={()=>{if(!deleteMode)window.location.hash=`#person:${person.id}`}}>
        ...
        <div className="meta"><div className="person-title-line"><strong className="name">{person.name}</strong><span className={stage?'intimacy-badge':'intimacy-badge provisional'}>{stage?`第${stage}段階 · ${definition.title}`:'段階未設定'}</span><span className={`track-tag ${track}`}>{track==='romance'?'ロマンス':track==='work'?'仕事・人脈':'友情'}</span></div>
