...
export default function CommunicationPersonPage({ person, self, onSave, onBack }){
  ...
  const [convoFormOpen, setConvoFormOpen] = useState(false)
  ...
  useEffect(()=>{
    ...
    setConvoFormOpen(false)
    setConvoExpanded(false)
  }, [person?.id])
  ...
  const showStagePicker = stagePickerOpen || !stage
  const visibleLog = convoExpanded ? log : log.slice(0, 3)
  const hasMoreLog = log.length > 3
    ...
    <div className="person-page communication-page stage-page">
      ...
      {activeTab === 'relation' ? (
        <>
          ...
          <section className="communication-card current-stage-card">
            ...
            {definition ? (
              <div className="current-stage-summary">
                <span className="current-stage-n">{stage}</span>
            ...
            {definition && (
              <button type="button" className="stage-picker-toggle" aria-expanded={showStagePicker} onClick={()=> setStagePickerOpen(v => !v)}>
                {showStagePicker ? '段階の一覧を閉じる ▴' : '段階を変更する ▾'}
              </button>
            )}
            {showStagePicker && (
                ...
                {stages.map(item => {
                  ...
                  return (
                    <button type="button" role="radio" aria-checked={stage === item.id}
                      ...
                      key={item.id} onClick={()=> { chooseStage(item.id); setStagePickerOpen(false) }}>
          ...
          <section className="communication-card conversation-card">
            <div className="section-heading with-progress">
              <div><p className="eyebrow">CONVERSATION LOG</p><h3>会話の記録</h3></div>
              {log.length > 0 && <span>{log.length}件</span>}
            </div>
            ...
            {convoFormOpen ? (
              <div className="convo-form">
                ...
                <div className="convo-form-actions">
                  <button type="button" className="advance" disabled={!draft.topics.trim() && !draft.theirResponse.trim() && !draft.appreciated.trim() && !draft.wantToAsk.trim()} onClick={()=> { addConversation(); setConvoFormOpen(false) }}>この会話を記録する</button>
                  <button type="button" className="secondary" onClick={()=> setConvoFormOpen(false)}>閉じる</button>
                </div>
              </div>
            ) : (
              <button type="button" className="convo-add-toggle" onClick={()=> setConvoFormOpen(true)}>＋ 会話を記録する</button>
            )}
            ...
            {hasMoreLog && (
              <button type="button" className="convo-more-toggle" onClick={()=> setConvoExpanded(v => !v)}>
                {convoExpanded ? '最新3件だけ表示' : `もっと見る（他${log.length - 3}件）`}
              </button>
            )}
          </section>
            ...
            <main className="stage-content">
              ...
              <section className="communication-card momentum-card">
                ...
                <div className="safety-inline">
                  <p className="safety-inline-label">次の一手の前に、相手にとっても心地よいか</p>
                  <div className="safety-inline-checks">
                    {SAFETY_ITEMS.map(([key, label])=> (
                      <label key={key} className={profile.nextStepSafety[key] ? 'safety-chip checked' : 'safety-chip'}>
              ...
              <section className="communication-card about-card">
                <div className="section-heading"><p className="eyebrow">ABOUT THEM</p><h3>この人について</h3></div>
                ...
                {collectionGuide.length > 0 && (
                  <div className="about-sub">
                    <div className="section-heading with-progress"><h4>この段階で知っておくこと</h4><span>{collectionFilled}/{collectionGuide.length}</span></div>
