...
import YamadaPractice from './YamadaPractice'
...
export default function SelfPage({self,onSave,onBack,saveStatus,onRetry}){
  ...
  return <div className="y-page">
    ...
    <YamadaPractice self={self} onSave={onSave}/>
    <details className="y-card"><summary>会う前に、気持ちを切り替える</summary><section className="y-mode">
    ...
    <details className="y-card"><summary>自動モードの設定・自分のプロフィール{self.gender?'（'+(self.gender==='other'?'その他':self.gender)+'）':''}</summary><SelfSettings self={self} onSave={onSave}/></details>
    ...
    <p className="y-footer">連続記録も、期限もありません。自分のペースで、一つずつ。</p>
  </div>
}
